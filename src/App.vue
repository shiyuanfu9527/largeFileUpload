<template>
  <div class="page">
    ...
    <div class="content" ref="contentRef">
      <listItem :task-arr="taskArr" @pauseUpdate="pauseUpdate" @goonUpdate="goonUpdate" @reset="reset"></listItem>
    </div>
...
import { AllDataItem, checkFile, clearDir, mergeSlice, taskArrItem, update } from './api/home';
import listItem from './listItem.vue';
// 显示到视图层的初始数据
const contentRef = ref()
const localForage = (getCurrentInstance()!.proxy as any).$localForage
const unit = 1024 * 1024 * 3 //每个切片的大小定为3m
...
// 监听任务改变
watch(() => taskArr.value, (_newVal, _oldVal) => {
  setTaskArr()
}, { deep: true })
onMounted(() => {
  getTaskArr()
...
const pauseUpdate = (item: taskArrItem, elsePause = true) => {
  // 先看是不是失败，如果不是elsePause为true，就是暂停，为false就是中断
...
const goonUpdate = (item: taskArrItem) => {
  ...
  item.allData.push(...item.whileRequest)
  item.whileRequest = []
...
// 全部取消
const clear = () => {
  for (const item of taskArr.value) {
    pauseUpdate(item)
  }
...
const inputChange = async (e: Event) => {
  ...
  for (let h = 0; h < files.length; h++) {
    ...
    let inTaskArrItem: taskArrItem = {
      ...
      fileSize: file.size,
      allData: [], // 所有请求的数据
      whileRequest: [], //正在请求的请求个数，目前是要永远都保存请求个数为6
      finishNumber: 0, //请求完成的个数
      errNumber: 0, //报错的个数，默认是0，超多3个就是直接上传中断
      percentage: 0
    }
    taskArr.value.push(inTaskArrItem)
    nextTick(() => {
      contentRef.value.scrollTop = (110 + 20) * (taskArr.value.length + 1)
    }) //设置滚动条滚到底部
    ...
    const sliceNumber = Math.ceil(file.size / unit) // 向上取整切割次数，例如20.52，就要为了剩余的0.52再多一次便利
    // 先看看是不是有同一个文件在上传中或者在暂停着
    // 再查本地再查远程服务器，本地以及上传了一半就重新切割好对上指定的片，继续上传就好，state 2是上传中，3是暂停中
    const isNeedUploadingArr = uploadingArr.filter(item => fileMd5 === item.md5 && item.state === 2)
    const theSameMd5Arr = toRaw(taskArr.value).filter(item => item.md5 === fileMd5) //和当前文件哈希值一样的文件进度
    ...
    if (theSameMd5Arr.length > 0 && theSameMd5Arr[0].state !== 4) {
      ...
      if (firstItem.state === 2) {
        message(`${firstItem.name}已经在上传中了`)
        ...
        updateObj.state = 2
        updateObj.allData.push(...updateObj.whileRequest)
        updateObj.whileRequest = []
      ...
      message(`${updateTaskObj.name}已经上传了部分，现在可以继续上传`)
      for (let i = 0; i < sliceNumber; i++) {
        const inFileMd5 = `${updateTaskObj.md5}-${i}`
        const inAllDateItem = updateTaskObj.allData.find((item) => item.fileMd5 == inFileMd5)
        inAllDateItem ? inAllDateItem.file = file.slice(i * unit, i * unit + unit) : ''
      }
...
const useWorker = (file: File) => {
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = (e) => {
      ...
      console.log(e);
      name === 'succedd' ? resolve(data) : reject(data)
    }
  })
}
//切片上传
const slicesUpdate = (taskArrItem: taskArrItem, progressTotal = 100) => {
  // 一片都没有了，或者有正在请求中的接口，都直接不执行下边的逻辑，毕竟都有正在请求中的还上创，容易高于浏览器并发数的限制
  if (taskArrItem.allData.length === 0 || taskArrItem.whileRequest.length > 0) {
    return
  }
  const isTaskArrIng = toRaw(taskArr.value).filter(itemB => itemB.state === 1 || itemB.state === 2)
  maxNumb = Math.ceil(6 / isTaskArrIng.length) //实时动态获取并发请求数，每次掉请求前都获取一次并发数
  ...
  taskArrItem.whileRequest.push(...whileRequest)
  ...
  async function isUpdate(needObj: AllDataItem) {
    ...
    if (taskArrItem.state === 5 || taskArrItem.state === 3) return //状态都已经暂停或者变成中断了，就什么都不要做了,及时中断
    // 请求异常，或者请求成功服务端返回错误都按单片上传失败逻辑处理，then.catch的.catch是只能捕获异常的
    if (!res || res.result === -1) {
      ...
      if (result === 1) {
        ...
        needObj.finish = true
        taskArrItem.whileRequest = taskArrItem.whileRequest.filter(item=>item.index!==needObj.index) //上传成功了就删掉请求数字中的那一片
        console.log('请求成功了:>> ',taskArrItem.whileRequest.length );
...
// 更新进度条
  ...
  const setTaskArr = async()=>{
    // localForage 这个库的api不兼容Proxy对象和函数，要处理一下
