...
router.post('/checkFile',cors(),(req,res)=>{
    try {
        ...
        res.send({result:-1,msg:'上传失败',data:error})
    }
})
...
router.post('/clearDir',cors(),(req,res)=>{
    try {
        ...
        const filesB = fs.readFileSync(cacheDir)
        for(const file of files){
            // 注意：git是不能接受空文件夹的，.gitignore文件相当于空文件夹的占位文件夹，不能删它，不然git会把你的空文件夹删掉
            file !== '.gitignore'?fs.unlinkSync(`${finishDir}/${file}`):''
        }
        for(const file of filesB){
            // node.js 不支持删除有文件的文件夹，这种情况下就只能递归处理
    ...
    fs.readdir(folderPath,(err,data)=>{
        if(!err){
            ...
            const addStream = (pathArr)=>{
                ...
                const buffer = fs.readFileSync(path) //根据file对象的路径获取file对象内容
                endWs.write(buffer)
